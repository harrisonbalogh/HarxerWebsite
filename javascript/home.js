...
document.getElementById('scene-home-info').onmouseenter = function() {
	TweenLite.to(document.getElementById('scene-home-info'), 0.5, {width: 304, height: 86});
};

document.getElementById('scene-home-info').onmouseleave = function() {
	TweenLite.to(document.getElementById('scene-home-info'), 0.75, {width: 0, height: 30, ease: Power4.easeInOut});
};
...
(function start() {
	...
	// var newWisp = new Wisp(canvas_bg.width/2, canvas_bg.height/2, canvasMasterContext);
	// wisps.push(newWisp);
  ...
  mouseTracker.onmousemove = function (e) {
		if (canvasRunning && mouseReady) {
			...
			// }
			if (selectedOrb == 4 && canvasDrawing) {
				if (mouse.px == 0 && mouse.py == 0) {
					Tunnel.tunnelBuilder.setPosition(mouse.x,mouse.y);
				}
				if (mouse.x != 0 && mouse.x != 0) {
					Tunnel.tunnelBuilder.setWaypoint(mouse.x,mouse.y);
				}
			}
  ...
  mouseTracker.oncontextmenu = function (e) {
	...
	// TweenLite.to(loaderOverlay, 0.4, {delay: 0, opacity: 0});
	// TweenLite.to(loaderText, 0.4, {delay: 0, opacity: 0});
